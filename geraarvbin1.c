...
void preOrder(TipoNo* root, TipoChave* pre, int* index) {
  if (root == NULL) return;
  pre[*index] = root->Reg.Chave;
  (*index)++;
  preOrder(root->Esq, pre, index);
  preOrder(root->Dir, pre, index);
}
  
void inOrder(TipoNo* root, TipoChave* in, int* index) {
  if (root == NULL) return;
  inOrder(root->Esq, in, index);
  in[*index] = root->Reg.Chave;
  (*index)++;
  inOrder(root->Dir, in, index);
}

void posOrder(TipoNo* root, TipoChave* pos, int* index) {
  if (root == NULL) return;
  posOrder(root->Esq, pos, index);
  posOrder(root->Dir, pos, index);
  pos[*index] = root->Reg.Chave;
  (*index)++;
}

void printInOrder(TipoChave* in, int size) {
...
void printPreOrder(TipoChave* pre, int size) {
...
void printPosOrder(TipoChave* pos, int size) {
  printf("Pos-Order Traversal: ");
  for (int i = 0; i < size; i++) {
    printf("%ld ", pos[i]);
  }
  printf("\n");
...
int main(int argc, char *argv[])
{
  ...
  TipoChave pre[prm.treesize], in[prm.treesize], pos[prm.treesize];
  int index = 0;
  // preOrder(root, pre, &index);
  index = 0;
  preOrder(root, in, &index);
  index = 0;
  posOrder(root, pos, &index);
